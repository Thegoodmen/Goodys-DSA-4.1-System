[![foundry-shield]][foundry-url]
[![Forks][forks-shield]][forks-url]
[![Stargazers][stars-shield]][stars-url]
[![Issues][issues-shield]][issues-url]

# Goodys-DSA-4.1-System

## Current Features
- Automated Combat with NPC Actors
- Importing Characters from Heldentool
- Loot Actors, Merchant  Sheets and Shoping System
- Support for dice-so-nice and pdf Integration

## Upcoming Features
- Support for Activ Effects for a number of Spells
- GM-Tools and Roll Tables
- Summening Integration (Demonic&Elemental)
- Weapon & Artefact  Smiting

## Links & Recommendations
- [Dice so Nice](https://gitlab.com/riccisi/foundryvtt-dice-so-nice) is an amazing addon with wonderfull 3D Dices and fully supported by the System
- [PDF Pager](https://github.com/farling42/fvtt-pdf-pager) is a helpfull Tool to have you PDFs inside Foundry ready

## Special Thanks
* [Yagrima](https://github.com/yagrima) for Testing, Feedback and listening to my rants while Developing!
* The Amazing People and their amazing feedback, over at the [Goody's DSA Corner](https://discord.gg/hFjcsX9QbF)!
* [STBaf](https://github.com/STBaf) for some Bugfixes that I missed or had not fixed yet
* [Geanofee]((https://github.com/geanofee) for working on the Integration to AutoAnimation for the nice Animations when doing anything in combat!
* The People at Ulisses Spiele for clearing me for creating this System as well as there Support!

## Copyright
DAS SCHWARZE AUGE, AVENTURIEN, DERE, MYRANOR, THARUN, UTHURIA, RIESLAND und THE DARK EYE sind eingetragene Marken der Ulisses Spiele GmbH, Waldems. 
Die Verwendung der Grafiken und Texte in dieser Software erfolgt unter den von Ulisses Spiele erlaubten Richtlinien. Eine Verwendung über diese Richtlinien 
hinaus darf nur nach vorheriger schriftlicher Genehmigung der Ulisses Medien und Spiel Distribution GmbH erfolgen.

[foundry-shield]: https://img.shields.io/badge/Foundry-v13.342-informational
[foundry-url]: https://foundryvtt.com/
[forks-shield]: https://img.shields.io/github/forks/Thegoodmen/Goodys-DSA-4.1-System.svg?style=flat-square
[forks-url]: https://github.com/Thegoodmen/Goodys-DSA-4.1-System/network/members
[stars-shield]: https://img.shields.io/github/stars/Thegoodmen/Goodys-DSA-4.1-System.svg?style=flat-square
[stars-url]: https://github.com/Thegoodmen/Goodys-DSA-4.1-System/stargazers
[issues-shield]: https://img.shields.io/github/issues/Thegoodmen/Goodys-DSA-4.1-System.svg?style=flat-square
[issues-url]: https://github.com/Thegoodmen/Goodys-DSA-4.1-System/issues
